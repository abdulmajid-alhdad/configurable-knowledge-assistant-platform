...
class AssociationRepositoryPort(Protocol):
    def list_source_ids(self, *, assistant_id: AssistantId, workspace_id: WorkspaceId) -> frozenset[KnowledgeSourceId]: ...


class ConversationRepositoryPort(Protocol):
    def add(self, conversation: Conversation, *, workspace_id: WorkspaceId) -> None: ...
    def get(self, *, conversation_id: ConversationId, workspace_id: WorkspaceId) -> Conversation | None: ...
...
class AssistantConversationService:
    def __init__(self, *, assistants: AssistantRepositoryPort, sources: SourceRepositoryPort,
                 associations: AssociationRepositoryPort, conversations: ConversationRepositoryPort,
                 messages: MessageRepositoryPort, rag: DocumentRagService) -> None:
    ...
    def create(self, *, workspace_id: WorkspaceId, assistant_id: AssistantId) -> Conversation:
        ...
        conversation = Conversation.create_for_assistant(workspace_id=workspace_id, assistant_id=assistant_id)
        self._conversations.add(conversation, workspace_id=workspace_id)
        return conversation
    ...
    def ask(self, *, workspace_id: WorkspaceId, conversation_id: ConversationId,
        ...
        assistant = self._assistants.get(assistant_id=conversation.assistant_id, workspace_id=workspace_id)
